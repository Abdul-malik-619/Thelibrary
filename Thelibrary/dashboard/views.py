...
def notes(request):
    if request.method == "POST":
        ...
        if form.is_valid:
            ...
            notes.save()
        messages.success(request,f'Notes made by{request.user.username} Added Successfully')
    else:
        form = NotesForm()
    ...
    if request.method == "POST":
        ...
        for i in range(10):
            result_dict={
               ...
               'descritption' :answer['items'][i]['volumeInfo'].get('description'),
            ...
            if 'input' in request.POST:
                ...
                if input and int(input)>=0:
                    if first =='pound' and second =='kilogram':
                        answer =f'{input} pound ={int(input)*0.453593} kilogram'
                    if first =='kilogram' and second =='pound':
                        answer =f'{input} foot ={int(input)*2.20456} pound'
                context ={
                    'form':form,
                    'm_form':measurement_form,
                    'input':True,
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request,f"Account for {username} created succefully")
